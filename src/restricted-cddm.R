...
# Prior distribution: variance percived angle
  ...
  for(dd in 1:n_difficulty){
    tau_pos[dd] ~ dlnorm(mu_var_pos[dd], tau_var_pos)
...
# Prior distribution: variance of percived cue
  ...
  tau_tau_cue ~ dgamma(2, 1)

  for(aa in 1:n_abs_cues){
    tau_cue[aa] ~ dlnorm(mu_tau_cue, tau_tau_cue)
    var_cue[aa]    <- 1/sqrt(tau_cue[aa])
  }

  for(t in 1:n){
# Prior distribution: angles and mizture component
    z[t]            ~ dbern(omega[i[t]])
    theta_tmp2[t,1] ~ dnorm(position[t], tau_pos[d[t]])
